...
class TechnicalSeoAuditEngine
{
    ...
    public function register(): void
    {
        ...
        add_action(self::CRON_HOOK, [$this, 'runAudit']);
    }
    ...
    public function registerRoutes(): void
    {
        ...
        register_rest_route('ameverywhere/v1', '/audit/technical', [
            'methods'             => \WP_REST_Server::READABLE,
            'callback'            => fn() => rest_ensure_response(get_option(self::RESULTS_OPTION, null) ?: ['message' => 'No audit has been run yet.']),
            'permission_callback' => $adminCap,
        ]);

        register_rest_route('ameverywhere/v1', '/audit/technical/run', [
            'methods'             => \WP_REST_Server::CREATABLE,
            'callback'            => [$this, 'scheduleRun'],
            'permission_callback' => $adminCap,
        ]);

        register_rest_route('ameverywhere/v1', '/audit/technical/progress', [
            'methods'             => \WP_REST_Server::READABLE,
            'callback'            => fn() => rest_ensure_response(get_option(self::PROGRESS_OPTION, ['status' => 'idle', 'percent' => 0, 'current_check' => ''])),
            'permission_callback' => $adminCap,
        ]);
    }
    ...
    public function scheduleRun(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        update_option(self::PROGRESS_OPTION, ['status' => 'running', 'percent' => 0, 'current_check' => 'Starting…']);
        return rest_ensure_response(['success' => true, 'message' => 'Audit scheduled.']);
    }
    ...
    public function runAudit(): void
    {
        ...
        foreach ($checks as $checkId => $label) {
            ...
            try {
                ...
                $items  = method_exists($this, $method) ? $this->{$method}() : [];
            } catch (\Throwable $e) {
        ...
        $results['summary']      = ['critical' => $critical, 'warnings' => $warnings, 'passed' => $passed];
    ...
    private function missingMetaDescriptions(): array
    {
        $query = new \WP_Query([
            ...
            'meta_query'     => [
                'relation' => 'OR',
                ['key' => '_ameverywhere_meta_description', 'compare' => 'NOT EXISTS'],
                ['key' => '_ameverywhere_meta_description', 'value' => '', 'compare' => '='],
            ],
        ]);

        $items = [];
        foreach ($query->posts as $post) {
            $items[] = ['post_id' => $post->ID, 'title' => $post->post_title, 'url' => get_permalink($post->ID)];
        }
        return $items;
    }
    ...
    private function missingAltText(): array
    {
        ...
        return $count > 0 ? [['count' => $count, 'message' => "$count images are missing alt text."]] : [];
    }
    ...
    private function check404Errors(): array
    {
        ...
        return $count > 0 ? [['count' => $count, 'message' => "$count 404 errors logged."]] : [];
    }
    ...
    private function orphanedPages(): array
    {
        $query = new \WP_Query([
            ...
            'posts_per_page' => 10,
            'meta_query'     => [
                ['key' => '_ameverywhere_inbound_links', 'value' => '0', 'compare' => '=', 'type' => 'NUMERIC'],
            ],
        ]);
        ...
        return $total > 0 ? array_merge([['total_orphaned' => $total]], array_map(
            fn($p) => ['post_id' => $p->ID, 'title' => $p->post_title, 'url' => get_permalink($p->ID)],
            $query->posts
        )) : [];
    }
    ...
    private function noindexImportantPages(): array
    {
        ...
        if ($frontPageId > 0) {
            ...
            if ($noindex === 'yes') {
                $issues[] = ['post_id' => $frontPageId, 'title' => 'Front Page', 'issue' => 'Front page is set to noindex!'];
            }
        }
    ...
    private function sitemapHealth(): array
    {
        ...
        if (is_wp_error($response)) {
            return [['issue' => 'Sitemap unreachable: ' . $response->get_error_message()]];
        }

        $code = wp_remote_retrieve_response_code($response);
        return ($code === 200) ? [] : [['issue' => "Sitemap returned HTTP $code."]];
    }
    ...
    private function robotsTxt(): array
    {
        ...
        if (is_wp_error($response)) {
            return [['issue' => 'robots.txt unreachable: ' . $response->get_error_message()]];
        }

        $body = wp_remote_retrieve_body($response);
        if (stripos($body, 'Disallow: /') !== false && strpos($body, 'Disallow: /wp-admin') === false) {
            return [['issue' => 'robots.txt may be blocking all crawlers. Review carefully.']];
        }
    ...
    private function favicon(): array
    {
        ...
        return empty($favicon) ? [['issue' => 'No site icon (favicon) is set. Set one in Appearance → Customize → Site Identity.']] : [];
    }
    ...
    private function sslCertificate(): array
    {
        ...
        return (strpos($siteUrl, 'https://') !== 0)
            ? [['issue' => 'Site is not using HTTPS. SSL is a confirmed Google ranking signal.']]
            : [];
    }
    ...
    private function schemaPresence(): array
    {
        $query = new \WP_Query([
            ...
            'order'          => 'DESC',
            'meta_query'     => [
                ['key' => '_ameverywhere_schema_type', 'compare' => 'NOT EXISTS'],
            ],
        ]);

        return $query->found_posts > 0
            ? [['count' => $query->found_posts, 'message' => $query->found_posts . ' recent posts have no schema type set.']]
            : [];
    }
    ...
    {
        $stale = (array) get_option('ameverywhere_stale_cornerstone_ids', []);
        return empty($stale) ? [] : [['count' => count($stale), 'message' => count($stale) . ' cornerstone posts are stale.']];
    }
    ...
    private function deriveStatus(string $checkId, array $items): string
    {
        ...
        $criticalChecks = ['noindex_important_pages', 'ssl_certificate', 'robots_txt'];
        $warningChecks  = ['missing_meta_descriptions', 'redirect_chains', 'check_404_errors', 'sitemap_health', 'duplicate_titles', 'stale_cornerstone'];
