...
class AdminModule
{
    ...
    {
        register_rest_route('ameverywhere/v1', '/settings', [
            [
        ...
        register_rest_route('ameverywhere/v1', '/settings/ai/test-connection', [
            'methods'             => \WP_REST_Server::CREATABLE,
        ...
        register_rest_route('ameverywhere/v1', '/pagespeed', [
            'methods'             => \WP_REST_Server::READABLE,
        ...
        register_rest_route('ameverywhere/v1', '/gsc/keywords', [
            'methods'             => \WP_REST_Server::READABLE,
            'callback'            => [$this, 'getGscKeywords'],
            'permission_callback' => [$this, 'checkPermission'],
            'args'                => [
                'post_id' => [
                    'required'          => true,
        ...
        register_rest_route('ameverywhere/v1', '/gsc/dashboard', [
            'methods'             => \WP_REST_Server::READABLE,
            'callback'            => [$this, 'getGscDashboard'],
            'permission_callback' => [$this, 'checkPermission'],
        ]);

        // ── Phase 2 Backlog: Keyword Cannibalization Report ──────────────────
        register_rest_route('ameverywhere/v1', '/gsc/cannibalization', [
            'methods'             => \WP_REST_Server::READABLE,
            'callback'            => [$this, 'getKeywordCannibalization'],
            'permission_callback' => [$this, 'checkPermission'],
        ]);

        // ── Phase 2 Backlog: Orphan Pages Report ─────────────────────────────
        register_rest_route('ameverywhere/v1', '/links/orphans', [
            'methods'             => \WP_REST_Server::READABLE,
            'callback'            => [$this, 'getOrphanPages'],
            'permission_callback' => [$this, 'checkPermission'],
        ]);

        // ── Phase 2 Backlog: Site-wide Link Statistics ───────────────────────
        register_rest_route('ameverywhere/v1', '/links/stats', [
            'methods'             => \WP_REST_Server::READABLE,
            'callback'            => [$this, 'getLinkStats'],
            'permission_callback' => [$this, 'checkPermission'],
        ]);

        // ── Phase 2 Backlog: GA4 Integration settings ────────────────────────
        register_rest_route('ameverywhere/v1', '/settings/ga4', [
            [
                'methods'             => \WP_REST_Server::READABLE,
                'callback'            => [$this, 'getGa4Settings'],
                'permission_callback' => [$this, 'checkPermission'],
            ],
            [
                'methods'             => \WP_REST_Server::CREATABLE,
                'callback'            => [$this, 'updateGa4Settings'],
                'permission_callback' => [$this, 'checkPermission'],
            ],
        ]);

        // ── Phase 2 Backlog: GA4 Traffic Report ──────────────────────────────
        register_rest_route('ameverywhere/v1', '/ga4/report', [
            'methods'             => \WP_REST_Server::READABLE,
            'callback'            => [$this, 'getGa4Report'],
            'permission_callback' => [$this, 'checkPermission'],
        ]);

        // ── Phase 3: Import / Export settings ────────────────────────────────
        register_rest_route('ameverywhere/v1', '/settings/export', [
            'methods'             => \WP_REST_Server::READABLE,
            'callback'            => [$this, 'exportSettings'],
            'permission_callback' => [$this, 'checkPermission'],
        ]);

        register_rest_route('ameverywhere/v1', '/settings/import', [
            'methods'             => \WP_REST_Server::CREATABLE,
        ...
        register_rest_route('ameverywhere/v1', '/html-sitemap', [
            [
                'methods'             => \WP_REST_Server::READABLE,
                'callback'            => [$this, 'getHtmlSitemapSettings'],
                'permission_callback' => [$this, 'checkPermission'],
            ],
            [
                'methods'             => \WP_REST_Server::CREATABLE,
                'callback'            => [$this, 'saveHtmlSitemapSettings'],
                'permission_callback' => [$this, 'checkPermission'],
            ],
        ]);

        register_rest_route('ameverywhere/v1', '/html-sitemap/preview', [
            'methods'             => \WP_REST_Server::READABLE,
            'callback'            => [$this, 'previewHtmlSitemap'],
            'permission_callback' => [$this, 'checkPermission'],
        ]);
    }
    ...
    public function manualIndexUrl(\WP_REST_Request $request): \WP_REST_Response|\WP_Error
    {
        ...
        if (!$url) {
            return new \WP_Error('invalid_post', __('Invalid Post ID.', 'ameverywhere'), ['status' => 400]);
        }

        // Fire job in the background securely via WP Cron
        $queueManager = new \AmEveryWhere\Core\Queue\QueueManager();
        $queueManager->push(\AmEveryWhere\Modules\Indexing\IndexingJob::class, [
            'post_id' => $postId,
            'url'     => $url,
            'action'  => 'URL_UPDATED'
        ]);
    ...
    public function getPageSpeed(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        if (!$url) {
            return new \WP_Error('invalid_post', __('Could not resolve post URL.', 'ameverywhere'), ['status' => 404]);
        }
        ...
        if ($cached) {
            ...
            if (isset($data['fetched_at']) && (time() - $data['fetched_at']) < WEEK_IN_SECONDS) {
                return rest_ensure_response(['success' => true, 'scores' => $data, 'cached' => true]);
            }
        }
        ...
        return rest_ensure_response(['success' => true, 'scores' => $scores, 'cached' => false]);
    }
    ...
    public function getGscKeywords(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        if (is_wp_error($response)) {
            return new \WP_Error('gsc_request_failed', $response->get_error_message(), ['status' => 502]);
        }
        ...
        if (empty($data['rows'])) {
            ...
            update_post_meta($postId, '_ameverywhere_ranking_keywords', wp_json_encode([]));
            update_post_meta($postId, '_ameverywhere_ranking_keywords_at', time());
            return rest_ensure_response(['success' => true, 'keywords' => [], 'cached' => false]);
        }
        ...
        return rest_ensure_response(['success' => true, 'keywords' => $keywords, 'cached' => false]);
    }
    ...
    public function getGscDashboard(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        $trendData = $gscQuery([
            'startDate'  => $startDate, 'endDate' => $endDate,
            'dimensions' => ['date'], 'rowLimit' => 28,
            'orderBy'    => [['fieldName' => 'date', 'sortOrder' => 'ASCENDING']],
        ]);

        $totalClicks = 0; $totalImpressions = 0;
        $ctrSum = 0; $posSum = 0; $rowCount = 0;
        $topQueries = [];

        foreach ($queriesData['rows'] ?? [] as $row) {
    ...
    public function getKeywordCannibalization(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        if (!$accessToken) {
            return new \WP_Error('gsc_not_connected', __('Google Search Console not connected.', 'ameverywhere'), ['status' => 401]);
        }
        ...
        if (is_wp_error($response)) {
            return new \WP_Error('gsc_failed', $response->get_error_message(), ['status' => 502]);
        }
        ...
        $result = ['success' => true, 'conflicts' => array_slice($conflicts, 0, 50), 'total' => count($conflicts), 'cached' => false];
        set_transient($cacheKey, $result, 6 * HOUR_IN_SECONDS);
        return rest_ensure_response($result);
    }
    ...
    public function getOrphanPages(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        $cached   = get_transient($cacheKey);
        if ($cached !== false) {
            return rest_ensure_response(array_merge(['success' => true, 'cached' => true], $cached));
        }
        ...
        if ($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $linksTable)) !== $linksTable) {
            return rest_ensure_response(['success' => true, 'orphans' => [], 'total' => 0, 'orphan_count' => 0, 'linked_count' => 0]);
        }
        ...
        $linkedMap  = array_flip($linkedUrls ?: []);
        ...
        return rest_ensure_response(array_merge(['success' => true, 'cached' => false], $data));
    }
    ...
    public function getLinkStats(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        if ($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $table)) !== $table) {
            return rest_ensure_response(['success' => true, 'message' => __('Link index table not found. Save a post to initialize it.', 'ameverywhere'), 'stats' => null]);
        }
        ...
        $topTargets = array_map(function ($row) {
        ...
        $topSources = array_map(function ($row) {
    ...
    public function updateGa4Settings(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        if (isset($params['credentials']) && !empty($params['credentials'])) {
            ...
            if ($decoded && isset($decoded['private_key'], $decoded['client_email'])) {
                ...
                return new \WP_Error('invalid_credentials', __('The provided credentials JSON is not a valid Google Service Account file.', 'ameverywhere'), ['status' => 422]);
            }
        }
    ...
    public function getGa4Report(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        if (!$credentials || !isset($credentials['private_key'], $credentials['client_email'])) {
            return new \WP_Error('ga4_invalid_credentials', __('Invalid GA4 credentials.', 'ameverywhere'), ['status' => 422]);
        }

        // Build JWT for GA4 Data API scope
        $accessToken = $this->getGa4AccessToken($credentials);
        if (!$accessToken) {
            return new \WP_Error('ga4_auth_failed', __('Failed to obtain GA4 access token. Check your Service Account credentials.', 'ameverywhere'), ['status' => 401]);
        }
        ...
        $summary = ['sessions' => 0, 'users' => 0, 'pageviews' => 0, 'bounce_rate' => 0, 'avg_session' => 0];
        ...
        $channels = [];
        foreach ($channelsRaw['rows'] ?? [] as $row) {
            $channels[] = [
    ...
    public function importSettings(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        if (empty($params['settings']) || !is_array($params['settings'])) {
            return new \WP_Error('invalid_payload', __('Missing or malformed settings object.', 'ameverywhere'), ['status' => 422]);
        }

        if (($params['_plugin'] ?? '') !== 'ameverywhere') {
            return new \WP_Error('wrong_plugin', __('This settings file was not exported from AmEveryWhere.', 'ameverywhere'), ['status' => 422]);
        }
    ...
    public function previewHtmlSitemap(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        foreach ($postTypes as $postType) {
            ...
            $items = array_map(fn($p) => [
                ...
                'children' => [], // hierarchical support can be added in v2
            ], $posts);
