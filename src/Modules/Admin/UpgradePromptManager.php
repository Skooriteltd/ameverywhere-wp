...
class UpgradePromptManager
{
    ...
    private const PRO_FEATURES = [
        'writing_assistant'      => ['name' => 'AI Writing Assistant',       'tier' => 'pro'],
        'content_gap'            => ['name' => 'Content Gap Analysis',       'tier' => 'pro'],
        'rank_tracker'           => ['name' => 'Keyword Rank Tracker',       'tier' => 'pro'],
        'agency_workspace'       => ['name' => 'Multi-Brand Workspaces',     'tier' => 'enterprise'],
        'schema_aggregation'     => ['name' => 'Schema Competitor Import',   'tier' => 'pro'],
        'ai_visibility_tracker'  => ['name' => 'AI Visibility Tracker',     'tier' => 'enterprise'],
        'gsc_integration'        => ['name' => 'Google Search Console',      'tier' => 'pro'],
        'technical_audit'        => ['name' => 'Full Technical SEO Audit',   'tier' => 'pro'],
        'client_reports'         => ['name' => 'Client Reporting Portal',    'tier' => 'enterprise'],
        'bulk_csv_redirects'     => ['name' => 'Bulk CSV Redirects',        'tier' => 'pro'],
    ];

    public function boot(): void
    {
        add_action('admin_notices', [$this, 'showUpgradeBannerNotice']);
        add_action('rest_api_init', [$this, 'registerRoutes']);
        add_action('wp_ajax_ameverywhere_dismiss_upgrade_banner', [$this, 'handleDismissAjax']);
    }
    ...
    public function registerRoutes(): void
    {
        ...
        register_rest_route('ameverywhere/v1', '/upgrade/features', [
            'methods'             => \WP_REST_Server::READABLE,
            'callback'            => [$this, 'listFeatures'],
            'permission_callback' => $editorCap,
        ]);

        register_rest_route('ameverywhere/v1', '/upgrade/dismiss-banner', [
            'methods'             => \WP_REST_Server::CREATABLE,
            'callback'            => [$this, 'dismissBanner'],
            'permission_callback' => $editorCap,
        ]);

        register_rest_route('ameverywhere/v1', '/upgrade/check-feature', [
            'methods'             => \WP_REST_Server::CREATABLE,
    ...
    public function listFeatures(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        return rest_ensure_response(['current_tier' => $currentTier, 'features' => $features]);
    }
    ...
    public function checkFeature(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        if (!isset(self::PRO_FEATURES[$key])) {
            return new \WP_Error('unknown_feature', 'Unknown feature key.', ['status' => 404]);
        }
