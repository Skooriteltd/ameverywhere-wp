...
class MorphologicalKeywordMatcher
{
    ...
    {
        register_rest_route('ameverywhere/v1', '/content/keyword-density', [
            'methods'             => \WP_REST_Server::CREATABLE,
            'callback'            => [$this, 'analyseKeywordDensity'],
            'permission_callback' => fn() => current_user_can('edit_posts'),
        ]);

        register_rest_route('ameverywhere/v1', '/content/stem', [
            'methods'             => \WP_REST_Server::CREATABLE,
            'callback'            => [$this, 'stemEndpoint'],
            'permission_callback' => fn() => current_user_can('edit_posts'),
        ]);
    }
    ...
    public function analyseKeywordDensity(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        if (empty($keyword) || empty($content)) {
            return new \WP_Error('missing_input', 'keyword and content (or post_id) are required.', ['status' => 400]);
        }
    ...
    public function stemEndpoint(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        if (empty($word)) {
            return new \WP_Error('missing_word', 'word is required.', ['status' => 400]);
        }
