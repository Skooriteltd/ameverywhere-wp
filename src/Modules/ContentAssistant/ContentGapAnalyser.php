...
class ContentGapAnalyser
{
    ...
    {
        register_rest_route('ameverywhere/v1', '/content/gap-analysis', [
            'methods'             => \WP_REST_Server::CREATABLE,
            'callback'            => [$this, 'analyseGap'],
            'permission_callback' => fn() => current_user_can('edit_posts'),
        ]);

        register_rest_route('ameverywhere/v1', '/content/gap-drafts', [
            'methods'             => \WP_REST_Server::CREATABLE,
            'callback'            => [$this, 'createDraftPosts'],
            'permission_callback' => fn() => current_user_can('edit_posts'),
        ]);
    }
    ...
    public function analyseGap(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        if (empty($keyword)) {
            return new \WP_Error('missing_keyword', 'A target keyword is required.', ['status' => 400]);
        }
    ...
    public function createDraftPosts(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        if (empty($topics) || !is_array($topics)) {
            return new \WP_Error('missing_topics', 'topics array is required.', ['status' => 400]);
        }
        ...
        foreach (array_slice($topics, 0, 10) as $topic) {
            ...
            if (!is_wp_error($postId)) {
                ...
                $created[] = ['post_id' => $postId, 'title' => $title, 'edit_url' => get_edit_post_link($postId, 'raw')];
            }
        }
    ...
    private function askLlmForGapTopics(string $keyword, array $existing): array|\WP_Error
    {
        ...
        if (empty($apiKey)) {
            return new \WP_Error('no_api_key', 'No LLM API key configured. Set one in AmEveryWhere → AI Settings.', ['status' => 400]);
        }
