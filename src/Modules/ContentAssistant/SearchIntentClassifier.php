...
class SearchIntentClassifier
{
    ...
    private const SIGNALS = [
        'transactional' => [
            'buy', 'order', 'purchase', 'shop', 'checkout', 'discount', 'deal',
            'coupon', 'promo', 'sale', 'price', 'cost', 'cheap', 'affordable',
            '$', '£', '€', 'add to cart', 'get started', 'sign up', 'subscribe',
        ],
        'commercial' => [
            'best', 'top', 'review', 'reviews', 'vs', 'versus', 'compare',
            'comparison', 'alternative', 'alternatives', 'rating', 'recommend',
            'recommendation', 'pros and cons', 'worth it', 'is it good',
        ],
        'navigational' => [
            'login', 'log in', 'sign in', 'contact', 'about', 'homepage',
            'official', 'website', 'portal', 'dashboard', 'account',
        ],
        'informational' => [
            'how', 'what', 'why', 'when', 'where', 'who', 'which',
            'guide', 'tutorial', 'tips', 'learn', 'explained', 'introduction',
            'beginners', 'basics', 'overview', 'definition', 'example', 'examples',
            'step by step', 'how to', 'what is', 'what are',
        ],
    ];
    ...
    public function register(): void
    {
        ...
        add_action('save_post', [$this, 'classifyOnSave'], 20);
    }
    ...
    public function classifyOnSave(int $postId): void
    {
        ...
        if (!$post || !in_array($post->post_status, ['publish', 'draft'], true)) {
            return;
        }
    ...
    public function getConfidenceScore(int $postId): array
    {
        ...
        if (!$post) {
            return ['scores' => [], 'winner' => 'informational', 'total_signals' => 0];
        }
        ...
        foreach (self::SIGNALS as $intent => $signals) {
            foreach ($signals as $signal) {
                if (mb_strpos($text, mb_strtolower($signal)) !== false) {
                    $scores[$intent]++;
                    $found[] = ['intent' => $intent, 'signal' => $signal];
                }
            }
        }
    ...
    public function registerRoutes(): void
    {
        ...
        register_rest_route('ameverywhere/v1', '/content/intent', [
            'methods'             => \WP_REST_Server::READABLE,
            'callback'            => [$this, 'getIntent'],
            'permission_callback' => $editorCap,
            'args'                => ['post_id' => ['required' => true, 'sanitize_callback' => 'absint']],
        ]);

        register_rest_route('ameverywhere/v1', '/content/intent/bulk-classify', [
            'methods'             => \WP_REST_Server::CREATABLE,
            'callback'            => [$this, 'bulkClassify'],
    ...
    public function getIntent(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        return rest_ensure_response([
            ...
            'optimisation_tips'   => self::TIPS[$confidence['winner']] ?? [],
        ]);
    }
