...
class ImageSeoModule
{
    ...
    public function boot(): void
    {
        ...
        // Fix missing alt tags in post content on save (classic editor / raw HTML)
        $this->eventManager->addFilter('wp_insert_post_data', [$this, 'fixMissingAltTags'], 10, 2);

        // Fix missing alt tags in Gutenberg core/image block output at render time
        $this->eventManager->addFilter('render_block', [$this, 'fixGutenbergImageAlt'], 10, 2);
    ...
    public function fixGutenbergImageAlt(string $blockContent, array $block): string
    {
        ...
        // If block already has alt text in its attributes, trust it
        $altAttr = $block['attrs']['alt'] ?? '';
        if (!empty($altAttr)) {
            return $blockContent;
        }
        ...
        $blockContent = preg_replace_callback(
            ...
            function ($matches) use ($altText) {
                $attrs = $matches[1];
                // Only fix if alt is missing or empty
                if (preg_match('/\balt\s*=\s*"[^"]+"/i', $attrs)) {
                    return $matches[0]; // Already has non-empty alt
                }
                $safeAlt = esc_attr($altText);
                // Replace empty alt or insert
                if (preg_match('/\balt\s*=\s*""/i', $attrs)) {
                    $attrs = preg_replace('/\balt\s*=\s*""/i', "alt=\"{$safeAlt}\"", $attrs);
                } else {
